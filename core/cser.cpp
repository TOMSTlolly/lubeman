...
#define RMD_SECONDS 30  // po tolika sekundach odeslu RMD packet na odeslani chybejicich dat ....
#define PYT_RESTART 10
...
//
	...
	if (getadapternumber(fAdapterNumber)!=FT_OK)
	{
...
bool Csr::wait_for_line(const char *stim, char *Await)
{
	...
	} while ((ret <= 0) && (GetTickCount() - StartTime < 3500));
...
#define PROC_PATH "/proc/"
#define CMDLINE_FILE "/cmdline"
void Csr::start_process(const char *process_name)
{
   if (fork() == 0)
   {
     printf("start_process %s \r\n",process_name);
     execlp(process_name, process_name, NULL);  // Restart the process
     perror("Failed to restart process");
     exit(EXIT_FAILURE);
   }
}
...
int get_pids_by_namex(const char *process_name, int *pids, int max_pids) {
	...
	snprintf(command, sizeof(command), "ps aux | grep pyguard.py | awk '{print $2}'", process_name);
...
bool Csr::stop_all_instances(const char *process_name) {
	int pids[256];
...
void remove_ftdi_module() {
	int result = system("rmmod ftdi_sio");
	if (result == 0) {
		printf("ftdi_sio module removed successfully.\n");
	} else {
		perror("Failed to remove ftdi_sio module");
	}
}
...
int Csr::mLoop()
{
	...
	char cmd[42];
	char adr[21]; // popis chyby adapter, vetsinou nejde adapter vypnout
	...
	static u_int64_t PyTime = 0;
	const char *lan_reader_cfg = "../lan_reader_cfg.txt";
	FT_STATUS ft_stat;
	bool adaActive = false;
	bool switchAda = false;  // zmena stavu adapteru
	iAdaErr  = 0;
	iWaitCnt = 0;
	...
	do
	{
		...
		case m_start:
			...
			PyTime = GetTickCount(); // zacatek mereni doby zivota pythoniho skriptu


			remove_ftdi_module();
			iret = open_adapter();
			if (iret>0) {
				ft_stat = FT_OTHER_ERROR;
				fMachineState = m_adapterError;  // hned po open_adapter
				break;
			}
			// mam k dispozici i cislo zkracene cislo adapteru z open_adapter
			logPrintf("Cislo adapteru:%s\n", fAdapterNumber);
			// je to zotaveni z restartu ?
			if (iAdaErr>0) {
				iAdaErr  = 0;
				iWaitCnt = 0;
				fMachineState = m_adapterToPython;
				break;
			}
			//char srv[] = "10.0.0.115";
			//int  port  = 5000;
		...
		case m_checkserver:
			...
			if (ret)
			{
				// ok, dostali jsme odpoved z pythoniho programu
				PyTime = GetTickCount(); // posledni znama znacka z pythonu
		...
		case m_checkmissing:
            ...
            // kdyz se mi nevraci nic na RMD, vyhlasim error
			// z nejakeho duvodu se mi vraci kazdy druhy packet
			// tak abych nepadal porad do blikani
			if (iSat++ > SATCNT)
			{
			  fMachineState = m_serverError;
			  printf("iSat: %d\r\n",iSat);
...
//			fMachineState = m_waitforserver;
		...
		case m_waitforadapter:
			...
			// sem se dostanu, kdyz zacina pruser s adapterem
			if ( (ft_stat = _poweroff()) != FT_OK)
			{
				//sprintf(adr,"ADA %d",ft_stat); // koduju cislo chyby
				fMachineState = m_adapterError;  // m_waitforadapter
				iSat = 0;
				break;
			}
			fMachineState = m_adapterToPython;
		...
		case m_adapterError:
#ifdef DEBUG
			logPrintf("m_adapterError \r\n");
#endif
			// UCELEM je shozeni programu, kdyz nefunguje adapteru
			// kdyz odeslu zpravu serveru, ze je adapter mrtvola
			sprintf(cmd,"ADS %s %d \r\n","ERROR",iret);
			ret = wait_for_line(cmd, line);
			ret = wait_for_line(cmd, line);
			if (ret == TRUE) {
				// vratila se mi odpoved ze serveru
				// restartuju okamzite, pokud mi server pozehna
				logPrintf("m_adapterError->%s\r\n", line);
			}
			if (iAdaErr>100) {
				logPrintf("**** COMMITING SUICIDE **** \r\n");
				exit(0);
			}
			...
			strcpy(fAdapterUser,"ERROR");
			fMachineState = m_adapterToPython;
			break;

		case m_adapterToPython:
#ifdef DEBUG
				logPrintf("m_adapterToPython \n");
#endif
				// UCELEM je NASTAVIT promennou adapteru v python
				// zkracene cislo adapteru posilam jenom do PYTHONU
				sprintf(cmd, "ADA %s \r\n", fAdapterUser);  // ADA ... send compressed adapter number to python
				ret = wait_for_line(cmd, line);
				if (ret > 0) {
					logPrintf("ADA->%s\r\n", line);
					if (iAdaErr>0) {
		...
		case m_servertime:
#ifdef DEBUG
			// ze serveru stahnu aktualni cas a nastavim do systemu
		...
		case m_prepdelp3:
			...
			else
			{
				...
				{
					sprintf(adr,"ADA %d",ft_stat); // koduju cislo chyby
					fMachineState = m_adapterError;  // m_prepdelp3
					break;
				}
