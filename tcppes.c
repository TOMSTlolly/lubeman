...
int sendData(int sockfd, int x) {
	int n;

	char buffer[32];
	//sprintf( buffer, "%d\n", x );
	sprintf(buffer, "CHK 123456\r\n", x);
	if ((n = write(sockfd, buffer, strlen(buffer))) < 0)
		return (1);  // vracim chybu
	buffer[n] = '\0';
	return (0);
}
...
int send_cmd(char *serverIp, int portno, char *cmd)
{
	int sockfd, n;
	//char serverIp[] = "10.0.0.115";
...
int conn_test(char* ServerIp, int port, int seconds)
{
	...
	addr_s.sin_addr.s_addr = inet_addr(ServerIp);
	...
	case 1: // data to read
		...
		if (so_error == 0) {
			...
			// odesli check
			if(sendData(fd, 1) > 0)
			  return 2;   // socket otevren, ale spatna odpoved (neni zde jiny server ?)
			
			return(0);   // OK, spravna odpoved
			break;
		}
		else {
			// error
